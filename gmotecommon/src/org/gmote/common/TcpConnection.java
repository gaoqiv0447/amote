...
import org.gmote.common.packet.ExceptionPacket;
...
public class TcpConnection {
  ...
  private static final int MAX_OLD_SESSION_IDS = 3;
  ...
  private ISuccessResponse successResponse = null;

  private String sessionId = null;

  private static Hashtable<String,Boolean> sessionState = new Hashtable<String,Boolean>(MAX_OLD_SESSION_IDS);
  /**
  ...
  public TcpConnection(AuthenticationHandler authHandler,ISuccessResponse successResponse) {
	    this.authHandler = authHandler;
  ...
  private void handleConnectionEstablished(Socket connectionSocket,
    ...
    LOGGER.info("==============!!!connectionSocket keepalive: "+String.valueOf(this.connectionSocket.getKeepAlive()));
  ...
  private Thread startPacketReceivingThread() {
    ...
    LOGGER.info("==============Connection state: "+String.valueOf(isConnected()));
    LOGGER.info("==============receiverThread is null: "+ (receiverThread == null));
    if(receiverThread!=null){
        addToTable(this.getSessionId(),true);
        LOGGER.info("==============receiverThread is not null: session id = "+ (this.getSessionId()));
    }
    receiverThread.start();
    return receiverThread;
  }

  public void addToTable(String sessionid, boolean state){

      if(sessionState.size() > MAX_OLD_SESSION_IDS){
          Object key =  sessionState.keySet().toArray()[0];
          sessionState.remove(key);
      }
      if(!sessionState.contains(sessionid)){
          sessionState.put(sessionid, state);
      }else{
          sessionState.put(sessionid, true);
      }
      for(String str:sessionState.keySet()){
          if(!str.equals(sessionid)){
              sessionState.put(str, false);
          }
      }
    ...
    try {
      ...
      if (result.getCommand() != Command.SUCCESS) {
        throw new AuthenticationException();
    	  ...
    	  successResponse.CheckSuccess(result);
      }

      if (!authHandler.isVersionCompatible(challenge.getServerVersion())) {
        throw new ServerOutOfDateException("The Amote server is out of date. Server Version: " + challenge.getServerVersion() + " Client Version: " + authHandler.getAppVersion(), challenge.getServerVersion());
      }
  ...
  private boolean handleServerSideAuthentication(Socket connectionSocket, String password)
    ...
    try {
      ...
      sendPacket(new SimplePacket(Command.SUCCESS,ScreenInfo.width,ScreenInfo.height,ScreenInfo.densityDpi));
      sessionId = challenge;
      LOGGER.log(Level.INFO, "----->Authentication of client success: "
  ...
  public void sendPacket(AbstractPacket packet) throws IOException {
	  LOGGER.info("==============Packet --- packet" + packet);
	  LOGGER.info("==============connectionOutput" + connectionOutput);
    connectionOutput.writeObject(packet);
    LOGGER.info("==============client sendPacket --- sendPacket");
    if(packet.getCommand() == Command.BEATHEART)
        LOGGER.info("==============BEATHEART Packet Send");
    if(packet.getCommand() == Command.BEATHEART_REPLY)
        LOGGER.info("==============BEATHEART_Reply Packet Send");
  ...
  public class DataReceiver extends Thread {
    ...
    public void run() {
      ...
      try {
          LOGGER.info("---->TcpConnection#DataReceiver run sessionid="+ connection.getSessionId());
          if(sessionState ==null){
              LOGGER.info("---->TcpConnection#DataReceiver sessionState= null");
          }
          if(sessionState.get(connection.getSessionId()) ==null){
              LOGGER.info("---->TcpConnection#DataReceiver sessionState.get = null");
          }
        while (sessionState.get(connection.getSessionId())) {
          AbstractPacket packet = (AbstractPacket) connectionInput.readObject();
         // LOGGER.info("=============TcpConnection#DataReceiver packet receive!");
         // LOGGER.info("==============DataReceiver***Connection state: "+String.valueOf(isConnected()));
          receiver.handleReceiveData(packet, connection);
        }
      } catch (IOException e) {
          if(e instanceof java.io.EOFException){
              LOGGER.info("---->TcpConnection#DataReceiver run client exit");
          }
        receiver.handleReceiveData(new ExceptionPacket(Command.CLIENT_EXIT_FORCE), connection);
  ...
  public void closeConnection() {
    try {
      if (connectionSocket != null) {
        connectionSocket.close();
        LOGGER.info("---->TcpConnection#connectionSocket CLOSED!!!");
      }
