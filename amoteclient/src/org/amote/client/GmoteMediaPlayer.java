...
public class GmoteMediaPlayer {
  ...
  private synchronized void fetchSong() {
    ...
    try {
      ...
      if (sessionId == null) {
        String message = Resources.getSystem().getString(R.string.fetch_song_error);
        Log.i(ActivityUtil.DEBUG_TAG, message);
        updateListener.sendMessage(Message.obtain(updateListener, SESSION_ERROR, Resources.getSystem().getString(R.string.cant_request_file)));
        return;
      }
